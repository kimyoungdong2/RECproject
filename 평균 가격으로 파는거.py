...
for one in range(len(Standard_Deviation_List)):
    ...
    for two in range(len(in_advance_List)):
        ...
        for three in range(10):
            First_Quantitative_Volume = Randoms_Price(First_Number, First_Average, Standard_Deviation) #정량은 정규분포(1)
            ...
            First_Middle_Price=sum(First_Quantitative_Price)/len(First_Quantitative_Price)            
            Second_Middle_Price=sum(First_Quantitative_Price)/len(First_Quantitative_Price)
            Third_Middle_Price=sum(First_Quantitative_Price)/len(First_Quantitative_Price)
            
            First_Middle_Sum=in_advance*sum(First_Middle_Price for i in range(len(First_All)))   # First_Middle_Sum = 몇 %를 먼저 구할것인지.
            Second_Middle_Sum=in_advance*sum(Second_Middle_Price for i in range(len(Second_All)))  #나중에 이 부분을 바꾸면 됨@@@@@@@@@@
            Third_Middle_Sum=in_advance*sum(Third_Middle_Price for i in range(len(Third_All)))
