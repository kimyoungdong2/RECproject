    ...
    for three in range(10):
        ...
        for i in range(len(All_of_first_second_third)):  #Down_for_efficiency에 가격이 낮은거(Quantitative_Price가 큰거) * 그에 맞는 양(Volumn)을 최대한 넣는 작업
            if tmp_for_efficiency< config.DATA_OF_2017['First_Capacity']+config.DATA_OF_2017['Second_Capacity']+config.DATA_OF_2017['Third_Capacity']: #총 판매 용량을 넘으면 안 됨.
                ...
                Down_for_volumn_efficiency=All_of_first_second_third[i][1]
        ...
        First_Sum = []
        for i in range(config.DATA_OF_2017['First_Number']):
            if sum(First_Sum) <(config.DATA_OF_2017['First_Capacity']) * 1.3:   #미리 산 용량은 뺀 후 1.3배
                try:
        ...
        for i in range(len(First_Middle_Pass) + 1):                     #1.3배가 아닌 1배로 다시 정렬
            if sum(First_Sum) < config.DATA_OF_2017['First_Capacity']: #정렬한 것들을 1배안에 드는 최종선정자들을 결정하는 for문
                try:
        ...
        for i in range(len(First_fail_plus_second)):
            if sum(Second_Sum) < (config.DATA_OF_2017['Second_Capacity']) * 1.3:   #미리 산 용량은 뺀 후 1.3배
                try:
        ...
        for i in range(len(Second_Middle_Pass) + 1):                 #1.3배가 아닌 1배로 다시 정렬
            if sum(Second_Sum) < config.DATA_OF_2017['Second_Capacity']: #정렬한 것들을 1배안에 드는 최종선정자들을 결정하는 for문
                try:
        ...
        for i in range(len(First_fail_plus_second_fail_plus_third)):
            if sum(Third_Sum) < (config.DATA_OF_2017['Third_Capacity']) * 1.3:   #미리 산 용량은 뺀 후 1.3배
                try:
        ...
        for i in range(len(Third_Middle_Pass) + 1):                 #1.3배가 아닌 1배로 다시 정렬
            if sum(Third_Sum) < config.DATA_OF_2017['Third_Capacity']: #정렬한 것들을 1배안에 드는 최종선정자들을 결정하는 for문
                try:
        ...
        All_of_volume=0
        for i in range(len(First_Final_Pass)):
            All_of_volume+=First_Final_Pass[i][1]
        for i in range(len(Second_Final_Pass)):
            All_of_volume+=Second_Final_Pass[i][1]
        for i in range(len(Third_Final_Pass)):
            All_of_volume+=Third_Final_Pass[i][1]

        All_of_first_second_third.sort()
        The_lowest_Quantitative_price=All_of_first_second_third[0][0]   
        Up_for_efficiency = All_of_pass    #Up_for_efficiency = 총 성공량(즉, 가격*양) + (가장 낮은 가격 * 미리 선정해둔것들)
        Up_for_volume_efficiency = All_of_volume
        config.efficiency+=Up_for_efficiency/Down_for_efficiency
        Volume_efficiency = Up_for_volume_efficiency/Down_for_volumn_efficiency
    config.ws.cell(row=2+(one*13),column=3).value=config.efficiency
    config.ws.cell(row=2+(one*13),column=4).value=config.Jain_fairness_index
    Percentage_of_scheduled_locations=(len(First_Get)+len(Second_Get)+len(Third_Get))/(len(First_All)+len(Second_All)+len(Third_All))
    config.ws.cell(row=2+(one*13),column=5).value=Percentage_of_scheduled_locations
    config.efficiency=0
